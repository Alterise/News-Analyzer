...
def train_model(model, train_loader, val_loader, device, is_combined=False, epochs=100):
    ...
    plt.title(f"{'Combined' if is_combined else 'Time Series'} Model Learning Curve")
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{'combined' if is_combined else 'timeseries'}_learning_curve.png")
    plt.close()
...
def main():
    ...
    ts_model, ts_train_loss, ts_val_loss = train_model(
        FairTimeSeriesModel(input_size=6).to(device), 
        ts_train_loader, 
        ts_test_loader, 
        device,
        epochs=epochs
    ...
    if news_data is not None:
        ...
        cmb_model, cmb_train_loss, cmb_val_loss = train_model(
            FairCombinedModel(ts_input_size=6, embedding_size=1024).to(device),
            cmb_train_loader,
            cmb_test_loader,
            device,
            is_combined=True,
    ...
    # Plot both learning curves together for comparison
    plt.figure(figsize=(12, 6))
    plt.plot(ts_train_loss, label='TS Train', color='blue', linestyle='-')
    plt.plot(ts_val_loss, label='TS Val', color='blue', linestyle='--')
    if news_data is not None:
        plt.plot(cmb_train_loss, label='Combined Train', color='red', linestyle='-')
        plt.plot(cmb_val_loss, label='Combined Val', color='red', linestyle='--')
