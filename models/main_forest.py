...
class NewsAggregator(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        # Apply mask to attention scores (set padding positions to very negative number)
...
def aggregate_embeddings_rnn(embeddings_df, device='cuda' if torch.cuda.is_available() else 'cpu'):
    """
    Aggregate embeddings by date using RNN with attention.
    
    Args:
        embeddings_df: DataFrame with 'timestamp', 'embedding_vector' columns
    """
    # Convert timestamps to date and sort by timestamp
    ...
    # Process each day's embeddings
    for date, group in tqdm(grouped, desc="Aggregating with RNN"):
        if len(group) == 1:
            # Only one news item for this day, use it directly
            date_embeddings.append((date, group['embedding_vector'].iloc[0]))
            continue
            
        # Stack embeddings into a tensor [seq_len, embedding_dim]
        day_embeddings = np.vstack(group['embedding_vector'].values)
        
        # Convert to tensor and move to device
        embeddings_tensor = torch.tensor(day_embeddings, dtype=torch.float32).unsqueeze(0).to(device)  # [1, seq_len, embedding_dim]
...
def train_rnn_aggregator(embeddings_df, embedding_dim, device='cuda' if torch.cuda.is_available() else 'cpu'):
    """
    Robust RNN aggregator training with comprehensive memory handling.
    """
    ...
    try:
        ...
        # 2. Tensor Conversion Phase
        print("Phase 2: Converting to tensors...")
        
        # Process sequences in smaller batches
        tensor_batches = []
        convert_batch_size = 100  # Small batches for tensor conversion
        ...
        # 3. Training Setup Phase
        print("Phase 3: Setting up training...")
        
        # Custom dataset with disk-backed storage if needed
        ...
        # Start with very small batch size
...
if __name__ == "__main__":
    ...
    # Visualization of data with t-SNE
